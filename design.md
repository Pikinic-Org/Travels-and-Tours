...
- **Hero:** the sky photo sits in an inset `rounded-2xl` frame behind a centred three-line title ("Fly further. / Travel smarter. / Book easier.") and the flight search bar. A top-to-bottom overlay (`neutral-900` 40% → 10%) keeps the white text legible over the bright clouds. The section keeps `z-20` and no `overflow-hidden`, so the airport and date pickers can hang over the next section.
...
- **Deep-green blocks** (the partnership call-to-action, the custom-package request) are inset `rounded-2xl` blocks with `DeepGreenBackdrop`. Content inside them sits directly on the block, with no nested cards. The partnership points are an icon (the 24px line style in a `green-500`/15% circle, icon in `green-500`), a heading and a line of text.
